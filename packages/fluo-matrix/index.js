...
function fluoMatrix(matrix, direct, presm) {
  presm = presm ?? this;
...
function fluoByRows(matrix, presm) {
  presm = presm ?? this;
  if (!matrix?.length || !presm) return matrix
  const serial = serialMatrix.call(presm, ROWWISE);
  return unflattenVector(serial, width(matrix))
}
function fluoByColumns(matrix, presm) {
  presm = presm ?? this;
  if (!matrix?.length || !presm) return matrix
  const serial = serialMatrix.call(presm, COLUMNWISE);
  return unflattenVector(serial, width(matrix))
}
function fluoByPoints(matrix, presm, width) {
  presm = presm ?? this;
  if (!matrix?.length || !presm) return matrix
...
export { fluoByColumns, fluoByPoints, fluoByRows, fluoMatrix };
