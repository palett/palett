...
const parseBound = bound => {
  ...
  if (nullish.valid(min)) {
    if (nullish.valid(dif)) return {
      min,
      dif
    };
    if (nullish.valid(max)) return {
      min,
      dif: max - min
    };
  }

  if (nullish.valid(dif)) {
    if (nullish.valid(max)) return {
      min: max - dif,
      dif
    };
...
const scale = (x, lo, lev, min, hi) => comparer.min((comparer.max(x, lo) - lo) * lev + min, hi);
/**
...
class ProjectorConfig {
  ...
  constructor(leapNum, leapHSL, napHSL, effects) {
    const dye$1 = dye.DyeFab.hsl.apply(null, effects);
    this.fab = dye$1.make.bind(dye$1);
    this.lo = leapNum.min;
  ...
  static fromHEX(bound, preset) {
    ...
    const max = (_preset$max = preset.max, convert.hexToHsl(_preset$max)),
          min = (_preset$min = preset.min, convert.hexToHsl(_preset$min)),
          nap = (_preset$na = preset.na, convert.hexToHsl(_preset$na)),
...
exports.ProjectorConfig = ProjectorConfig;
exports.scale = scale;
